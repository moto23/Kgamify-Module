...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
  ...
  if (isset($_POST['labelname'])) {
    ...
    $stmt = $conn->prepare("SELECT COUNT(*) FROM label WHERE label_name = ? and teacher_id=?");
        if ($stmt) {
            $stmt->bind_param("si", $label_name,$teacher_id);
            $stmt->execute();
            ...
            if ($count > 0) {
                echo "<script>alert('Label name already exists. Try editing the current Labels'); 
                window.location.href = 'all_labels.php';
                </script>";
            } else {
                // Use prepared statement to insert data safely
                $stmt = $conn->prepare("INSERT INTO label (teacher_id,label_name,status) VALUES (?,?,?)");

                // Check if the statement was prepared successfully
                if ($stmt) {
                  $stmt->bind_param("isi", $teacher_id,$label_name,$status);
                  ...
                  if ($execval) {
                    ...
                    $query="UPDATE label set unique_id='".$unique_id."' where label_id='".$last_id."'";
...
<body>
                  ...
                  <div class="container">
                    ...
                    <div id="contact">
                      ...
                      <fieldset>
                        ...
                        <input placeholder="Enter Label Name" id="labelname" type="text" name="labelname" tabindex="1" required autofocus />
                      </fieldset>
                      <br />
  ...
  <script>
    ...
    if (labelPattern.test(label)) {
        ...
        alert("Label Name should not contain any special characters or numbers");
    }
});
</script>
