...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    $stmt = $conn->prepare("SELECT COUNT(*) FROM department WHERE department_name = ? and institute_name = ?");
        if ($stmt) {
            $stmt->bind_param("ss", $department_name,$institute);
            $stmt->execute();
            ...
            if ($count > 0) {
                echo "<script>alert('Department name already exists for the selected Institute'); 
                window.location.href = 'add_department.php';
                </script>";
            } else {
                // Use prepared statement to insert data safely
                $institute_query = "SELECT institute_id,institute_abbreviation from institute where institute_name = ?";
                $institute_stmt = $conn->prepare($institute_query);
            
                if ($institute_stmt) {
                    $institute_stmt->bind_param("s", $institute);
                    $institute_stmt->execute();
                    $institute_stmt->bind_result($institute_id,$institute_abbr);
                    $institute_stmt->fetch();
                    $institute_stmt->close();
                }
                $insert_query = "INSERT INTO department (institute_id, institute_name, department_name) VALUES (?, ?, ?)";
                $insert_stmt = $conn->prepare($insert_query);
            
                if ($insert_stmt) {
                    $insert_stmt->bind_param("sss", $institute_id, $institute, $department_name);
                    $insert_stmt->execute();
                    $insert_stmt->close();
                    ...
                    $code=rand(1,9999);
                    $unique_id=$string."_".$institute_abbr."_".$code;
                    $query="UPDATE department set unique_id='".$unique_id."' where department_id='".$last_id."'";
...
<head>
    ...
    <title>NEW DEPARTMENT | PlayQuest</title>
...
<body>
                            ...
                            <form action="add_department.php" method="post">
                                <div class="card-header">
                                    <div class="container">
                                        ...
                                        <div id="contact">
                                            <fieldset>
                                                <p style="text-align: center;color: #ff9d5c; font-size: 30px;font-weight: bold; ">Add Department</p>
                                            ...
                                            <fieldset>
                                                ...
                                                ?>
                                                <select class="js-example-basic-single" name="institute" style="width: 100%;">
                                                    <?php
                                                ...
                                                //   $selected_institute = $_POST['institute_name'];
                                            ...
                                            <fieldset>
                                                <p style="color: #ff9d5c; font-size: 22px;font-weight: bold; ">Department Name</p>
                                                <input placeholder="Enter Department Name" id="department_name" type="text" name="department_name" tabindex="1" required autofocus />
                                            </fieldset>
                                            <br />

                                            <fieldset>

                                                <input type="submit" onclick="submit()" id="submit" value="Add Department" style=" font-size: 15px; padding: 10px 25px;background-color: #ff9d5c; color: white;border-radius: 8px;border: #ff9d5c;" />
    ...
    var departmentPattern = /[!@#$%^&*()+\=\[\]{};':"\\|,.<>\/?0-9]+/;

    if (departmentPattern.test(department)) {
        event.preventDefault(); // Prevent form submission
        alert("Department Name should not contain any special characters or numbers");
    }
});
</script>
