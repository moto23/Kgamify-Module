...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    // Check if category_name is set in the POST data
    if (isset($_POST['complexity']) && isset($_POST['coin_value'])) {
        ...
        $stmt = $conn->prepare("INSERT INTO champ_config (complexity,coin_value) VALUES (?,?)");

        // Check if the statement was prepared successfully
        if ($stmt) {
            $stmt->bind_param("ii", $complexity,$coin_value);
            ...
            if ($execval) {
                ...
                $query = "UPDATE champ_config SET unique_id='" . $unique_id . "' WHERE id='" . $last_id . "'";
        ...
        echo "<script>alert('Values not entered'); 
            window.location.href = 'champ_config.php';
            </script>";
    }
}
?>
...
<head>
  ...
  <title>CHAMP CONFIG | KGamify</title>
...
<body>
        ...
        <div class="row">
          <div class="col-md-12">
            <div class="card demo-icons">
              ...
              <!-- form start  -->
              <form action="champ_config.php" method="post">
                <div class="card-header">
                  <div class="container">
                    ...
                    <div id="contact">
                      <fieldset>
                        <p style="text-align: center;color: #ff9d5c; font-size: 30px;font-weight: bold; ">Add Champ Config</p>
                      ...
                      <fieldset>
                        <p style="color: #ff9d5c; font-size: 22px;font-weight: bold; ">Complexity Number</p>
                        <input placeholder="Enter Complexity Number" id="complexity" type="number" name="complexity" tabindex="1" required/>
                      </fieldset>
                      <br />
                      <fieldset>
                        <p style="color: #ff9d5c; font-size: 22px;font-weight: bold; ">Coin Value</p>
                        <input placeholder="Enter Coin Value" id="coin_value" type="number" name="coin_value" tabindex="1" required/>
                      </fieldset>
                      <br />

                      <fieldset>

                        <input type="submit" onclick="submit()" id="submit" value="Add Champ Config" style=" font-size: 15px; padding: 10px 25px;background-color: #ff9d5c; color: white;border-radius: 8px;border: #ff9d5c;" />
  ...
  <script>
    ...
    var coin_value = document.getElementById("complexity").value;
    var categoryPattern = /[!@#$%^&*()+\=\[\]{};':"\\|,.<>_-\/?a-bA-Z]+/;

    if (categoryPattern.test(complexity)) {
        event.preventDefault(); // Prevent form submission
        alert("Complexity Name should not contain any special characters or alphabets");
    }
    if (categoryPattern.test(coin_value)) {
        event.preventDefault(); // Prevent form submission
        alert("Coin Value should not contain any special characters or alphabets");
    }
});
</script>
</body>
