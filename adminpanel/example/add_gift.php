...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
  ...
  if (isset($_POST['gift_type'])) {
    ...
    $gift_desc = strip_tags($_POST['g_text']);
    $gift_icon = $_POST['gimg'];
    ...
    $stmt = $conn->prepare("INSERT INTO gift (gift_type,gift_description,gift_icon) VALUES (?,?,?)");

    // Check if the statement was prepared successfully
    if ($stmt) {
      $stmt->bind_param("sss", $gift_type,$gift_desc,$gift_icon);
      ...
      if ($execval) {
        ...
        $code=rand(1,9999);
        $unique_id=$string."_".$gift_type."_".$code;
        $query="UPDATE gift set unique_id='".$unique_id."' where gift_id='".$last_id."'";
...
<body>
  <div class="wrapper">
              ...
              <form action="add_gift.php" method="post">
                <div class="card-header">
                  <div class="container">
                    ...
                    <div id="contact">
                      <fieldset>
                        <p style="text-align: center;color: #ff9d5c; font-size: 30px;font-weight: bold; ">New Gift</p>
                      ...
                      <form action="add_gift.php" method="post" enctype="multipart/form-data">
    <fieldset>
        <p style="color: #ff9d5c; font-size: 22px; font-weight: bold;">Gift Type</p>
        <input placeholder="Enter Gift Type (Coupon /Gift Card /Product)" id="gift_type" type="text" name="gift_type" tabindex="1" required autofocus />
    </fieldset>
    <br>
    <fieldset>
        <p style="color: #ff9d5c; font-size: 22px; font-weight: bold;">Gift Description</p>
        <textarea name="g_text" id="editor" placeholder="Write Description here...."></textarea>
    </fieldset>
    <br />
    <fieldset>
        <p style="color: #ff9d5c; font-size: 22px; font-weight: bold;">Add Gift Icon</p>
        <input type="file" id="img" name="gimg" accept="image/*" required>
    </fieldset>
    <br>
    <input type="submit" id="submit" value="Add Gift" style="font-size: 15px; padding: 10px 25px; background-color: #ff9d5c; color: white; border-radius: 8px; border: #ff9d5c;" />
</form>
    ...
    var gift = document.getElementById("gift_type").value;
    var giftPattern = /[!@#$%^&*()+\=\[\]{};':"\\|,.<>\/?0-9]+/;

    if (giftPattern.test(gift)) {
        event.preventDefault(); // Prevent form submission
        alert("Gift Type should not contain any special characters or numbers");
    }
});
</script>
<script>
  CKEDITOR.replace( 'g_text' );
