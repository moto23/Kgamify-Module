...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
  ...
  if (isset($_POST['coin_value'])) {
    ...
    $stmt = $conn->prepare("SELECT COUNT(*) FROM coin_value WHERE coin_value = ?");
        if ($stmt) {
            $stmt->bind_param("s", $coin_value);
            $stmt->execute();
            ...
            if ($count > 0) {
                echo "<script>alert('Coin Value exists'); 
                window.location.href = 'coin_value.php';
                </script>";
            } else {
                // Use prepared statement to insert data safely
                $stmt = $conn->prepare("INSERT INTO coin_value (coin_value) VALUES (?)");

                // Check if the statement was prepared successfully
                if ($stmt) {
                    $stmt->bind_param("i", $coin_value);
    ...
    echo "<script>alert('Coin Value not entered'); 
                window.location.href = 'coin_value.php';
                </script>";
  }
}
...
<head>
  ...
  <title>NEW Coin Value | KGamify</title>
...
<body>
  <div class="wrapper">
        ...
        <div class="row">
          <div class="col-md-12">
            <div class="card demo-icons">
              ...
              <!-- form start  -->
              <form action="coin_value.php" method="post">
                <div class="card-header">
                  <div class="container">
                    ...
                    <div id="contact">
                      <fieldset>
                        <p style="text-align: center;color: #ff9d5c; font-size: 30px;font-weight: bold; ">New Coin Value</p>
                      ...
                      <fieldset>
                        ...
                        <input placeholder="Enter Coin Value" style="width: 150px;height:35px;" id="coin_value" type="number" name="coin_value" tabindex="1" required autofocus />
                      </fieldset>
                      <br />
    ...
    if (coinPattern.test(coin_value)) {
        event.preventDefault(); // Prevent form submission
        alert("Coin Value should not contain any special characters or alphabets");
    }
    else if(isNaN(coin_value) ||coin_value<0 || coin_value>10){
        event.preventDefault(); // Prevent form submission
        alert("Coin Value should be a number between 1 to 10");
    }
});
</script>
