...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
  ...
  if (isset($_POST['complexity'])) {
    ...
    $stmt = $conn->prepare("SELECT COUNT(*) FROM complexity WHERE complexity_value = ?");
        if ($stmt) {
            $stmt->bind_param("i", $complexity);
            $stmt->execute();
            ...
            if ($count > 0) {
                echo "<script>alert('Complexity Value already exists'); 
                window.location.href = 'complexity.php';
                </script>";
            } else {
                // Use prepared statement to insert data safely
                $stmt = $conn->prepare("INSERT INTO complexity (complexity_value) VALUES (?)");
    ...
    echo "<script>alert('Complexity not entered'); 
                window.location.href = 'complexity.php';
                </script>";
  }
}
...
<head>
  ...
  <title>NEW Complexity | KGamify</title>
...
<body>
  <div class="wrapper">
        ...
        <div class="row">
          <div class="col-md-12">
            <div class="card demo-icons">
              ...
              <!-- form start  -->
              <form action="complexity.php" method="post">
                <div class="card-header">
                  <div class="container">
                    ...
                    <div id="contact">
                      <fieldset>
                        <p style="text-align: center;color: #ff9d5c; font-size: 30px;font-weight: bold; ">New Complexity Value</p>
                      ...
                      <fieldset>
                        <p style="color: #ff9d5c; font-size: 22px;font-weight: bold; ">Complexity</p>
                        <input placeholder="Enter Complexity Value" style="width: 200px;height:35px;" id="complexity" type="number" name="complexity" tabindex="1" required autofocus />
                      </fieldset>
                      <br />
    ...
    if (coinPattern.test(complexity)) {
        event.preventDefault(); // Prevent form submission
        alert("Complexity should not contain any special characters or alphabets");
    }
    else if(isNaN(complexity) ||complexity<0 || complexity>10){
        event.preventDefault(); // Prevent form submission
        alert("Complexity should be a number between 1 to 10");
    }
});
</script>
