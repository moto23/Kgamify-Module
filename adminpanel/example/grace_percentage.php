...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
  ...
  if (isset($_POST['grace_percentage'])) {
    $grace_percentage = $_POST['grace_percentage'];
    ...
    $stmt = $conn->prepare("SELECT COUNT(*) FROM grace_percentage WHERE grace_percentage = ?");
        if ($stmt) {
            $stmt->bind_param("i", $grace_percentage);
            $stmt->execute();
            ...
            if ($count > 0) {
                echo "<script>alert('Percentage already exists'); 
                window.location.href = 'grace_percentage.php';
                </script>";
            } else {
                // Use prepared statement to insert data safely
                $stmt = $conn->prepare("INSERT INTO grace_percentage (grace_percentage) VALUES (?)");
    ...
    echo "<script>alert('Percentage not entered'); 
                window.location.href = 'grace_percentage.php';
                </script>";
  }
}
...
<head>
  ...
  <title>NEW Grace Percentage | KGamify</title>
...
<body>
  <div class="wrapper">
        ...
        <div class="row">
          <div class="col-md-12">
            <div class="card demo-icons">
              ...
              <!-- form start  -->
              <form action="grace_percentage.php" method="post">
                <div class="card-header">
                  <div class="container">
                    ...
                    <div id="contact">
                      <fieldset>
                        <p style="text-align: center;color: #ff9d5c; font-size: 30px;font-weight: bold; ">New Grace Percentage</p>
                      ...
                      <fieldset>
                        <p style="color: #ff9d5c; font-size: 22px;font-weight: bold; ">Grace Percentage</p>
                        <input placeholder="Enter Grace Percentage" style="width: 200px;height:35px;" id="grace_percentage" type="number" name="grace_percentage" tabindex="1" required autofocus />
                      </fieldset>
                      <br />

                      <fieldset>

                        <input type="submit" onclick="submit()" id="submit" value="Add Value" style=" font-size: 15px; padding: 10px 25px;background-color: #ff9d5c; color: white;border-radius: 8px;border: #ff9d5c;" />
    ...
    var grace_percentage = document.getElementById("grace_percentage").value;
    var coinPattern = /[!@#$%^&*()+=\[\]{};':"\\|,.<>\/?a-zA-Z]+/;

    if (coinPattern.test(grace_percentage)) {
        event.preventDefault(); // Prevent form submission
        alert("Grace Percentage should not contain any special characters or alphabets");
    }
    else if(isNaN(grace_percentage) || grace_percentage<0 || grace_percentage>100){
        event.preventDefault(); // Prevent form submission
        alert("Grace Percentage should be a number between 1 to 100");
    }
});
</script>
