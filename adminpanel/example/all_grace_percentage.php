...
$res = mysqli_query($conn, "select * from grace_percentage");
if ($conn->connect_error) {
  die("Connection failed: " . $conn->connect_error);
}

if (isset($_POST['updategracepercent'])) {
  $grace_percentage = $_POST['grace_percentage'];
  $grace_id = $_POST['grace_id'];
  
  $stmt = $conn->prepare("SELECT COUNT(*) FROM grace_percentage WHERE grace_percentage = ? AND grace_id != ?");
    $stmt->bind_param("si", $grace_percentage, $grace_id); // Assuming `id` is the unique identifier for coins
    $stmt->execute();
    ...
    if ($count > 0) {
        ...
        echo '<script>alert("Grace Percentage already exists!!");window.history.back();</script>';
        exit();
    } else {
  $query = "UPDATE grace_percentage set grace_percentage='$grace_percentage' where grace_id='$grace_id'";
  $query_run = mysqli_query($conn, $query);

  if ($query_run) {
    echo "<script>alert('Value Updated');window.location.href='all_grace_percentage.php';</script>";
  } else {
...
<head>
  ...
  <title>ALL Grace Percentages | KGamify</title>
        ...
        <div class="modal-dialog">
          <div class="modal-content">
            <div class="modal-header">
              <h5 class="modal-title">Edit Grace Percent</h5>
              <button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Close"></button>
            </div>
            <form action="all_grace_percentage.php" method="POST">
              <div class="modal-body">
                <input type="hidden" name="srno" id="srno">
                <input type="hidden" name="grace_id" id="grace_id">
                <div class="form-group">
                  <label>Grace Percentage</label>
                  <input type="text" class="form-control" id="grace_percentage" name="grace_percentage">
                </div>
              </div>
              <div class="modal-footer">
                ...
                <button type="submit" name="updategracepercent" class="updatebtn" style="font-size: 15px; padding: 5px 8px;background-color:#ff9d5c;border: #ff9d5c;border-radius:5px;color:white;font-weight:bold;">Update</button>
              </div>
            </form>
          </div>
        </div>
                        ...
                        <tbody>
                          ...
                          ?>
                            <tr>
                              ...
                              <td><?php echo $row['grace_percentage'] ?></td>
                            </tr>
                          <?php } ?>
                        </tbody>
                      </table>
