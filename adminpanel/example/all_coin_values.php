...
$res = mysqli_query($conn, "select * from coin_value");
...
if (isset($_POST['updatecoin'])) {
  ...
  $stmt = $conn->prepare("SELECT COUNT(*) FROM coin_value WHERE coin_value = ? AND coin_id != ?");
    $stmt->bind_param("si", $coin_value, $coin_id); // Assuming `id` is the unique identifier for coins
    $stmt->execute();
    ...
    if ($count > 0) {
        ...
        echo '<script>alert("Coin Value already exists!!");window.history.back();</script>';
        exit();
    } else {
  $query = "UPDATE coin_value set coin_value='$coin_value' where coin_id='$coin_id'";
  $query_run = mysqli_query($conn, $query);

  if ($query_run) {
    echo "<script>alert('Value Updated');window.location.href='all_coin_values.php';</script>";
  } else {
...
<head>
  ...
  <title>ALL Coin Values | KGamify</title>
        ...
        <div class="modal-dialog">
          <div class="modal-content">
            <div class="modal-header">
              <h5 class="modal-title">Edit Coin Value</h5>
              <button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Close"></button>
            </div>
            <form action="all_coin_values.php" method="POST">
              ...
              <div class="modal-footer">
                ...
                <button type="submit" name="updatecoin" class="updatebtn" style="font-size: 15px; padding: 5px 8px;background-color:#ff9d5c;border: #ff9d5c;border-radius:5px;color:white;font-weight:bold;">Update</button>
              </div>
            </form>
          </div>
        </div>
