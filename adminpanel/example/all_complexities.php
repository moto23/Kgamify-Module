...
$res = mysqli_query($conn, "select * from complexity");
if ($conn->connect_error) {
  die("Connection failed: " . $conn->connect_error);
}

if (isset($_POST['updatecomplexity'])) {
  $complexity_value = $_POST['complexity_value'];
  $complexity_id = $_POST['complexity_id'];
  
  $stmt = $conn->prepare("SELECT COUNT(*) FROM complexity WHERE complexity_value = ? AND complexity_id != ?");
    $stmt->bind_param("si", $complexity_value, $complexity_id); // Assuming `id` is the unique identifier for coins
    $stmt->execute();
    ...
    if ($count > 0) {
        // Value already exists, redirect back with an error
        echo '<script>alert("Complexity Value already exists!!");window.history.back();</script>';
        exit();
    } else {
  $query = "UPDATE complexity set complexity_value='$complexity_value' where complexity_id='$complexity_id'";
  $query_run = mysqli_query($conn, $query);

  if ($query_run) {
    echo "<script>alert('Value Updated');window.location.href='all_complexities.php';</script>";
  } else {
...
<head>
  ...
  <title>ALL Complexities | KGamify</title>
        ...
        <div class="modal-dialog">
          <div class="modal-content">
            <div class="modal-header">
              <h5 class="modal-title">Edit Complexity Value</h5>
              <button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Close"></button>
            </div>
            <form action="all_complexities.php" method="POST">
              <div class="modal-body">
                <input type="hidden" name="srno" id="srno">
                <input type="hidden" name="complexity_id" id="complexity_id">
                <div class="form-group">
                  <label>Complexity Value</label>
                  <input type="text" class="form-control" id="complexity_value" name="complexity_value">
                </div>
              </div>
              <div class="modal-footer">
                ...
                <button type="submit" name="updatecomplexity" class="updatebtn" style="font-size: 15px; padding: 5px 8px;background-color:#ff9d5c;border: #ff9d5c;border-radius:5px;color:white;font-weight:bold;">Update</button>
              </div>
            </form>
          </div>
        </div>
              ...
              <div class="card-header">
                <div class="container">
                  ...
                  <div id="contact">
                    ...
                    <fieldset>
                      <p style="text-align: center;color: #ff9d5c; font-size: 30px;font-weight: bold; ">All Coin Values</p>
                      ...
                      <table id="myTable" class="table-striped cell-border">
                        ...
                        <tbody>
                          ...
                          ?>
                            <tr>
                              <td><?php echo $a++ ?></td>
                              <td><?php echo $row['complexity_id'] ?></td>
                              <td><a class="editbtn">
                              ...
                              <td><?php echo $row['complexity_value'] ?></td>
                            </tr>
                          <?php } ?>
                        </tbody>
                      </table>
    ...
    <script>
      $(document).ready(function() {
        $('.editbtn').on('click', function() {
          ...
          $('#srno').val(data[0]);
          $('#complexity_id').val(data[1]);
          $('#complexity_value').val(data[3]);
          $('#action').val(data[2]);
        });
      });
    </script>
